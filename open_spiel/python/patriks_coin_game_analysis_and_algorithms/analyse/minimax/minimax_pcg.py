...
def value_function(state):
    """
    Value function to evaluate non-terminal states.
    Since we're searching to terminal states, this can return 0.
    Alternatively, implement a heuristic if needed.
    """
    if state.is_terminal():
        return state.player_return(maximizing_player_id=0)  # Assuming player 0 is MAX
    return 0  # Neutral value for non-terminal states

def main():
    # Load your custom game
    game = pyspiel.load_game("patriks_coin_game")

    # Convert the game to a sequential (turn-based) format
    ...
    # Since the game has at most 3 rounds, depth=6 (each round has two moves)
    maximum_depth = 6  # Adjust as necessary

    # Identify the maximizing player (assuming player 0 is MAX)
    maximizing_player_id = 0

    # Perform Expectiminimax search
    value, best_action = expectiminimax(
    ...
    # Translate the action ID to a readable string
    action_str = sequential_game.action_to_string(initial_state.current_player(), best_action)

    print(f"Optimal Value for Player {maximizing_player_id}: {value}")
    print(f"Best Action for Player {maximizing_player_id}: {action_str}")
